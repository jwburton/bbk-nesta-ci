...
binary_model <- function(N = 16,
  ...
  truth <- as.numeric(rbinom(1, 1, BR))
...
png("fig0.png", res = 500, width = 15, height = 11, units = "cm")
all_eval %>%
  ...
  labs(x = "Measure", 
       y = "Difference", 
       fill = "Network Treatment")+
  facet_nested(speci ~ sensi, labeller = label_both)+
  theme(
...
me_eval <- (static[12:18] - meanex[12:18])/static[12:18]
pol_eval <- (static[12:18] - polarize[12:18])/static[12:18]
sch_eval <- (static[12:18] - scheduled[12:18])/static[12:18]
...
all_eval %>%
  ...
  filter(variable == "CES" | variable == "AIES" | variable == "VAR") %>%
  ggplot(aes(x = variable, y = M*(-1), fill = condition))+
  geom_bar(alpha=0.7,stat = "identity", color = "black", position = position_dodge(width=0.9))+
  geom_errorbar(aes(ymin = (M-SD)*(-1), ymax = (M+SD)*(-1)), width = 0.2, position = position_dodge(width=0.9))+  
  geom_hline(yintercept = 0, size = 0.5, linetype=1, color = "black")+
  theme_bw()+
  scale_fill_manual(values=c("brown3", "cyan4", "azure3"))+
  scale_x_discrete(name ="Measure", limits=c("CES","AIES","VAR"))+
  labs(x = "Measure", 
       y = "Relative Difference", 
       fill = "Rewiring",
       title = "Relative Effects of Rewiring Compared to Matched Static Networks")+
  facet_nested(speci ~ sensi, labeller = label_both)+
  theme(
    panel.spacing = unit(0,"line"),
...
# zoomed in relative error change plot
all_eval %>%
  select(condition, sensi, speci, BR, GT, CES, CAE, CSRE, AIES, AIAE, AISRE, VAR) %>%
  melt(id = c("condition", "sensi", "speci", "BR", "GT")) %>%
  group_by(variable, condition, sensi, speci) %>%
  ...
  labs(x = "Measure", 
       y = "Relative Difference", 
       fill = "Rewiring",
       title = "Relative Effects of Rewiring Compared to Matched Static Networks [Zoomed In]")+
  facet_nested(speci ~ sensi, labeller = label_both)+
  theme(
    panel.spacing = unit(0,"line"),
    plot.title = element_text(face="bold", size = 10),
