...
res_full <- NULL
for (i in 1:500){
  output <- foreach(i = 1:57) %do% {numeric_model(task_index = i)}
...
### read in and prep omega data ###
omega_df <- read.csv("prior_tasks_with_omega_robustness.csv")

# get tasks' omega value
task_omegas <- omega_df %>%
  filter(n_agents == 20 & omega_as_frac == 0.33) %>%
  select(task_id, Omega_hat)
...
# join skewness and omega data
task_characteristics <- inner_join(task_omegas, task_skewness, by = "task_id")

# create task list - matches names with numbers used in simulation results df
task_list <- data.frame("task" = 1:57,
                        "task_id" = unique(emp_df$task_id))

# join omegas with task list
task_list <- inner_join(task_list, task_characteristics, by = "task_id")

# join task_list with simulated competition results
task_winners <- inner_join(task_winners, task_list, by = "task")

# inspect the spread of omega in the 57 estimation tasks considered.
p1 <- task_characteristics %>%
  ggplot(aes(x=Omega_hat))+
...
# plot interventions favored omega regions
p2 <- task_winners %>%
  filter(variable=="CES") %>%
  filter(condition != "static") %>% # we filter out static because it has <3 tasks
  ggplot(aes(x = Omega_hat, color = condition))+
...
# get descriptives for each condition's omega distribution
task_winners %>%
  group_by(condition, variable) %>%
  summarise(M_omega = mean(Omega_hat), SD_omega = sd(Omega_hat), n_tasks = n(), .groups = "drop") %>%
  filter(variable == "CES") %>% as.data.frame()

# Save plots as high-res png.
#png("omega_space.png", res = 500, width = 16, height = 6, units = "cm")
plot_grid(p1, p2, ncol = 2, rel_widths = c(0.65, 1), align = "h", labels = "AUTO")
#dev.off()

### inspect effects across omega ###
...
p3 <- noncomm_diff %>%
  filter(variable == "CES") %>%
  ggplot(aes(x=Omega_hat, y = M, color = condition, group = condition))+
  ...
  labs(x="Ω", y = "Effect on CES", color="Network Condition")
p3


### inspect relative effects across omega ###
...
get_rel_effects <- function(condition, reference, name_string){
  ...
  output$condition <- name_string
  output$task <- condition$task
  output$iteration <- rep(c(1:500), each = 57)
  output <- output %>% 
    select(-condition) %>%
...
scheduled_noncomm_rel <- get_rel_effects(scheduled, noncomm, "scheduled")

# bind, melt, summarise over iterations, and join with task info
noncomm_rel <- rbind(mean_extreme_noncomm_rel, polarize_noncomm_rel, static_noncomm_rel, scheduled_noncomm_rel)
noncomm_rel <- noncomm_rel %>% 
  group_by(task, condition, variable) %>% 
  summarise(M=mean(rel_eff), .groups="drop")
...
p4 <- noncomm_rel %>%
  ...
  theme_light()+
  labs(x="Ω", y = "Relative Effect on CES", 
       color = "Network Condition", fill = "Network Condition")
p4

### look at the "probability of improvement" across omega ###
...
p5 <- noncomm_prob %>%
  filter(variable=="CES") %>%
  ggplot(aes(x = Omega_hat, y = prob_improve, color = condition, group = condition))+
  geom_point(alpha=0.3)+
  geom_smooth(method = "loess", se = F)+
  labs(x="Ω", y="Probability of Improvement", color = "Network Condition")+
  scale_color_manual(labels = c("mean_extreme" = "Mean-Extreme","polarize" = "Polarize",
                                "scheduled" = "Scheduled", "static" = "Static"),
                     values=c("firebrick", "darkgoldenrod1", "cornflowerblue", "azure4"))+
  ylim(0,1)+
...
# create panel of omega-related plots
...
#png(filename = "omega_plots.png", res = 500, width = 16, height = 8, units = "cm")
plot_grid(prow, my_leg, nrow = 2, rel_heights = c(1, 0.1))
#dev.off()


### investigate skewness as a heuristic ###

# skewness distribution in tasks considered
p6 <- task_characteristics %>%
...
# omega x skewness correlation
task_characteristics %>%
  ggplot(aes(x=Omega_hat, y=skewness))+
  geom_point(alpha = 0.7)+
  geom_smooth(method = "lm")+
  theme_bw()+
  labs(x = "Ω", y = "Skewness")+
  annotate(x=0.4, y=30, geom="text",
           label=paste("R = ", 
                       round(cor(task_characteristics$Omega_hat, 
                                 task_characteristics$skewness),3)))+
  annotate(x=0.4, y=28, geom="text",
           label=paste("p = ", 
                       round(cor.test(task_characteristics$Omega_hat, 
                                      task_characteristics$skewness)$p.value,3)))
...
plot_grid(p6,p7, ncol = 2, rel_widths = c(0.65, 1), align = "h", labels = "AUTO")
#dev.off()
...
my_leg <- get_legend(p8+theme(legend.position = "bottom"))
prow <- plot_grid(p8+theme(legend.position = "none"),
